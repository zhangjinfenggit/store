 ...
 * 
 * @author zhangjinfeng
 * @date 2017年6月1日下午7:54:51 TODO
 */
public class InStoreReportPanel {

	private InStoreService service = null;
	List<InStore> list = null;
	...
	public JPanel createPieChart3D() {
		...
		if (list != null) {
			...
			for (InStore inStore : list) {
				dataset.setValue(inStore.getWeight(), inStore.getWareName(), inStore.getJoinTime());
			}
		}

		JFreeChart chart = ChartFactory.createBarChart("商品入库图", "商品名", "日期", dataset, PlotOrientation.VERTICAL, true,
				true, true);
